...
import '../utils/glute_bridges_analyzer.dart';

class GluteBridgesScreen extends StatefulWidget {
  const GluteBridgesScreen({super.key});

  @override
  State<GluteBridgesScreen> createState() => _GluteBridgesScreenState();
}

class _GluteBridgesScreenState extends State<GluteBridgesScreen> {
  ...
  final GluteBridgesAnalyzer _analyzer = GluteBridgesAnalyzer();
  ...
  late ValueNotifier<double> _kneeAngleNotifier;
  ...
  void initState() {
    ...
    _kneeAngleNotifier = ValueNotifier(0.0);
  ...
  void _speak(String text) async {
    if (text.isEmpty ||
        text == 'Waiting...' ||
        text == 'Hold and squeeze glutes!' ||
        text == 'Ready. Lift hips.') {
      return;
    }
  ...
  void _processPose(Pose pose, int imgWidth, int imgHeight) {
    ...
    final knee = _getLandmarkPoint(
      lm,
      side == 'right' ? PoseLandmarkType.rightKnee : PoseLandmarkType.leftKnee,
      imgWidth,
      imgHeight,
    );
    final ankle = _getLandmarkPoint(
      lm,
      side == 'right'
          ? PoseLandmarkType.rightAnkle
          : PoseLandmarkType.leftAnkle,
    ...
    final shoulderHipKneeAngle = GluteBridgesAnalyzer.calculateAngle(
      shoulder,
      hip,
      knee,
    );
    final hipKneeAnkleAngle = GluteBridgesAnalyzer.calculateAngle(
      hip,
      knee,
      ankle,
    );
    ...
    if (result.isBadForm ||
        result.statusMessage.contains('higher') ||
        result.statusMessage.contains('arch') ||
        result.statusMessage.contains('feet') ||
    ...
    _angleNotifier.value = result.bridgeAngle;
  }
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Text(
                            'AI Fitness Trainer - Glute Bridges',
                            style: TextStyle(
                              fontSize: 18,
