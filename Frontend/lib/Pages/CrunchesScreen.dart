...
import '../utils/crunches_analyzer.dart';
...
class _CrunchesScreenState extends State<CrunchesScreen> {
  ...
  final CrunchesAnalyzer _analyzer = CrunchesAnalyzer();
  ...
  void _speak(String text) async {
    if (text.isEmpty || text == 'Waiting...' || text == 'Hold and squeeze!') {
      return;
    }
  ...
  void _processPose(Pose pose, int imgWidth, int imgHeight) {
    ...
    final crunchAngle = CrunchesAnalyzer.calculateAngle(shoulder, hip, knee);
    final neckDist = CrunchesAnalyzer.euclideanDist(ear, shoulder);
    final isNoseVisible =
        lm[0] != null && lm[0]!.likelihood >= _minLandmarkConfidence;

    final result = _analyzer.analyze(
      shoulderHipKneeAngle: crunchAngle,
      neckDist: neckDist,
      noseVisible: isNoseVisible,
    );

    if (result.isBadForm ||
        result.statusMessage.contains('Pull') ||
        result.statusMessage.contains('Lift') ||
        result.statusMessage.contains('flat')) {
      _speak(result.statusMessage);
    }

    _repsNotifier.value = result.reps;
    _updateStatus(result.statusMessage, result.statusColor);
    _angleNotifier.value = result.crunchAngle;
  }
