...
import '../utils/lateral_raise_analyzer.dart';
import '../utils/pose_math.dart';

class LateralRaiseScreen extends StatefulWidget {
  const LateralRaiseScreen({super.key});

  @override
  State<LateralRaiseScreen> createState() => _LateralRaiseScreenState();
}

class _LateralRaiseScreenState extends State<LateralRaiseScreen> {
  ...
  final LateralRaiseAnalyzer _analyzer = LateralRaiseAnalyzer();
  ...
  LateralRaiseAnalysisResult? _latest;
  ...
  Future<void> _processCameraImage(CameraImage image) async {
    ...
    try {
      ...
      if (poses.isNotEmpty) {
        ...
        final computed = _computeLateralRaise(pose);
        if (computed != null) {
          final result = _analyzer.analyze(
            rawLElev: computed.lElev,
            rawRElev: computed.rElev,
            rawLForm: computed.lElbow,
            rawRForm: computed.rElbow,
  ...
  _LateralRaiseMetrics? _computeLateralRaise(Pose pose) {
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (_latest != null)
                                  Text(
                                    'L-ELEV: ${_latest!.lElevation.round()}   R-ELEV: ${_latest!.rElevation.round()}   TORSO: ${_latest!.torsoAngle.round()}',
                                    style: const TextStyle(
                                      color: Colors.white,
                                      fontSize: 12,
...
class _LateralRaiseMetrics {
  const _LateralRaiseMetrics({
    required this.lElev,
    required this.rElev,
